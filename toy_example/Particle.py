...
def evaluation_likelihood(mean, theta, sourcespace, data, exponent_like):
    ...
    log_likelihood = np.sum([log_normal(_d, np.exp(log_normal(sourcespace[idx], mean, 1)), theta) for idx, _d in enumerate(data)])
...
class Particle(object):
    def __init__(self, cfg=None):
        ...
        if 'theta_eff' not in cfg:
            raise ValueError('Error: set an estimate for the noise standard deviation')
        self.theta_eff = cfg['theta_eff']

        if 'method' not in cfg:
            raise ValueError('Error: select one of the three methods "FB" - "EM" - "PM"')
    ...
    def mh_mean(self, sourcespace, data, exponent_like):
        ...
        acceptance_prob = min(rapp_prior * rapp_like * rapp_proposal, 1)
    ...
    def mh_theta(self, sourcespace, data, exponent_like):
        ...
        rapp_prior = part_aux.prior / self.prior
        rapp_proposal = stats.gamma.pdf(self.theta, a=100, scale=part_aux.theta / 100) / stats.gamma.pdf(part_aux.theta, a=100, scale=self.theta / 100)
        rapp_like = part_aux.like / self.like
