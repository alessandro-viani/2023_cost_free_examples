function [result] = inverse_SESAME_prop_method(full_data, leadfield, sourcespace, cfg)
...
if isempty(noise_std)
    noise_std = 0.17 * max(max(abs(data)));%*sqrt(t_stop-t_start+1);
...
n_ist = size(data, 2);
if cfg.evol_exp > 0
    N = cfg.evol_exp+2;           % initial max number of iterations, determines array size
else
    N = 5000;
end
...
log_weights_unnorm = zeros(1,n_samples); %viani
All_log_Weights_u = zeros(N, n_samples); %viani
...
for i = 1:n_samples
    ...
    particle(i) = prior_and_like(particle(i), exponent_likelihood(1), ...
        leadfield, data, lambda_prior, dipmom_std, nsens, ncomp, fact, noise_std, n_ist); %viani
    ...
    log_weights_unnorm(i) = 0;
    %     log(1/sqrt(particle(i).like_det)^(n_ist*exponent_likelihood(1)) * ...
    %         exp(particle(i).log_like)^(exponent_likelihood(1)/(2*noise_std^2)));
end

log_update = zeros(1,n_samples);
w = max(log_weights_unnorm); %viani
log_cost_norm(n) = w + log(sum(exp(log_weights_unnorm - w)));
weights = exp(log_weights_unnorm - log_cost_norm(n));
ESS(n) = (sum(weights.^2))^-1;
All_log_Weights_u(n,:) = log_weights_unnorm;
...
while exponent_likelihood(n) <= 1
    [~, ind_max_weight] = max(weights);
    best_particle(n-1) = particle(ind_max_weight);

    log_weights_unnorm = log_weights_unnorm + log_update;
    w = max(log_weights_unnorm);
    log_cost_norm(n) = w + log(sum(exp(log_weights_unnorm-w))); %viani
    weights = exp(log_weights_unnorm - log_cost_norm(n));%viani
    ESS(n) = (sum(weights.^2))^-1;

    if max(isinf(log_weights_unnorm - (w + log(sum(exp(log_weights_unnorm - w)))))==1)%viani
    ...
    disp('----------------------------------------------------------------');
    disp(strcat(['Iteration ', num2str(n), ' -- Exponent = ', ...
        num2str(exponent_likelihood(n)), ' -- ESS = ', num2str(ESS(n)), ...
        ' -- Noise Std eff = ', num2str(noise_std / sqrt(exponent_likelihood(n))), ...
        ' -- log-Norm Cost = ', num2str(log_cost_norm(n))]));
    ...
    if ESS(n) < n_samples/2
        ...
        resampling_done(n) = 1;
        ESS(n) = n_samples;
        partition_weights = cumsum(weights);
        ...
        for i = 1:n_samples
            ...
            log_weights_unnorm(i) = log_cost_norm(n) - log(n_samples);
        end
    end
    ...
    for ii=1:1
        for i = 1:n_samples
            ...
            particle_proposed = prior_and_like(particle_proposed, exponent_likelihood(n), ...
                leadfield, data, lambda_prior, dipmom_std, nsens, ncomp, fact, noise_std, n_ist);

            log_rapp_like = 0.5*n_ist*(log(particle(i).like_det)-log(particle_proposed.like_det))+...
                0.5 * (exponent_likelihood(n)/(noise_std^2))*(particle(i).log_like-particle_proposed.log_like);
    ...
    if evol_exp == 0
        ...
        if exponent_likelihood(n) == 1
            ...
            while (log_ESS(n+1) - log(ESS(n))) > log(gamma_high) || (log_ESS(n+1) - log(ESS(n))) < log(gamma_low)
                ...
                for i=1:n_samples
                    if n < N
                        prop_part(i) = prior_and_like(prop_part(i), exponent_likelihood(n+1), ...
                            leadfield, data, lambda_prior, dipmom_std, nsens, ncomp, fact, noise_std, n_ist);
                        if exponent_likelihood(n) == 0
                            log_update(i) = n_ist * (- nsens * (0.5 * log(2*pi) + log(noise_std) - 0.5 * log(exponent_likelihood(n+1))) - 0.5 * log(prop_part(i).like_det)) - ...
                                0.5 * exponent_likelihood(n+1)/noise_std^2 * prop_part(i).log_like;
                        elseif exponent_likelihood(n) > 0
                            log_update(i) = n_ist * ( 0.5 * (log(particle(i).like_det) - log(prop_part(i).like_det)) + 0.5 * nsens *(log(exponent_likelihood(n+1)) - log(exponent_likelihood(n))))-...
                                1 / (2*noise_std^2) * (exponent_likelihood(n+1) * prop_part(i).log_like - exponent_likelihood(n) * particle(i).log_like);
                        end
                    end
                end
                log_weights_unnorm_exp = log_weights_unnorm + log_update;
                w = max(log_weights_unnorm_exp);
                log_cost_norm_aux = w + log(sum(exp(log_weights_unnorm_exp - w)));
                log_weight_aux = log_weights_unnorm_exp - log_cost_norm_aux;
                if max(isinf(log_weight_aux))==1
    ...
    for i=1:n_samples
        particle(i) = prior_and_like(particle(i), exponent_likelihood(n+1), leadfield, data, ...
            lambda_prior, dipmom_std, nsens, ncomp, fact, noise_std, n_ist);
    end
    n = n + 1;
end
n = n-1;
...
result.old_mod_sel = mod_sel;

in_it = 4;
noise_std_eff = noise_std ./ sqrt(exponent_likelihood(in_it:n));
delta = evaluate_delta(noise_std_eff);
n_samples_eff = (n-in_it)*n_samples;
All_log_Weights_u_eff = zeros(n_samples_eff, 1);
for j = 1:NDIP
    dipole(j) = struct('c', 0, 'qmean', zeros(3,1), 'qvar', zeros(3));
end

for i = 1:n_samples_eff
    particle_eff(i) = struct('nu',0, 'dipole',dipole, 'prior', 1, 'log_like', 0, 'like_det', 1, 'dipmom_std', 1);
end

count = 1;
for iter=in_it:n
    log_weight_upgrade = log(0.5) + log(delta(iter - in_it + 1)) -log(noise_std_eff(iter - in_it + 1)) + log_cost_norm(iter);
    for j=1:n_samples
        All_log_Weights_u_eff(count) = All_log_Weights_u(iter,j) +  log_weight_upgrade;
        particle_eff(count) = MCsamples{1, iter}.all_particles(j);
        count = count + 1;
    end
end

w = max(All_log_Weights_u_eff);
log_cost_norm_eff = w + log(sum(exp(All_log_Weights_u_eff-w)));
weights_eff = exp(All_log_Weights_u_eff - log_cost_norm_eff);

%ESTIMATES
[~, ind_max_weight] = max(weights_eff);
best_particle_eff = particle_eff(ind_max_weight);
mod_sel = zeros(NDIP+1,1);
pmap_old = pmap;
pmap = zeros(size(sourcespace,1), 1, NDIP);
for i = 1:n_samples_eff
    mod_sel(particle_eff(i).nu+1) = mod_sel(particle_eff(i).nu+1) + weights_eff(i);
    if particle_eff(i).nu <=NDIP
        for r = 1:particle_eff(i).nu
            pmap(particle_eff(i).dipole(r).c,1, particle_eff(i).nu) = ...
                pmap(particle_eff(i).dipole(r).c,1, particle_eff(i).nu) + weights_eff(i);
        end
    end
end
[~, ind_mod] = max(mod_sel);
disp(strcat(['Estimated number of dipoles: ', num2str(ind_mod-1)]))
[~, eee,pmap_singdip(:,:,1)] = point_estimation(particle_eff, weights_eff, sourcespace, NDIP);
kkk = 1;
for i = 1:numel(eee)
    est_dip(kkk,:) = [sourcespace(eee(i),:) 1 eee(i)];
    kkk=kkk+1;
    disp(strcat(['Estimated location of dipole ', num2str(i), ': vertex number ' num2str(eee(i))]));
end
if numel(est_dip)>0
    estimated_dipoles = est_dip(find(est_dip(:,4)==1),5);
    G_r = zeros(nsens,ncomp*numel(estimated_dipoles));
    for kk = 1:numel(estimated_dipoles)
        G_r(:,ncomp*(kk-1)+1:ncomp*kk) = leadfield(:,ncomp*(estimated_dipoles(kk)-1)+1:ncomp*estimated_dipoles(kk));
    end
...
log_noise_posterior =  log_cost_norm(in_it: end) - log(noise_std_eff);%log(gampdf(noise_std_eff, 2 ,noise_std * 4))
log_noise_posterior = log_noise_posterior - max(log_noise_posterior);

noise_post = exp(log_noise_posterior);
[~, best_iter] = max(noise_post);

%inutili allo stato attuale in quanto non interpolo, altrimenti questa
%iterazione potrebbe essere diversa da best_iter
[~, idx_max_val] = max(noise_post);
noise_map_prop = noise_std_eff(idx_max_val);

noise_cm_prop = 0;
I=0;
for i=2:size(noise_std_eff,2)
    I = I + 0.5*(noise_post(i-1) + noise_post(i)) * abs(noise_std_eff(i-1)-noise_std_eff(i));
end

for i=2:size(noise_std_eff,2)
    noise_cm_prop = noise_cm_prop + 0.5*(noise_std_eff(i-1)*noise_post(i-1) + noise_std_eff(i)*noise_post(i)) * abs(noise_std_eff(i-1)-noise_std_eff(i));
end

noise_cm_prop = noise_cm_prop/I;

disp(['Noise estimates: MAP: ', num2str(noise_map_prop), ' -- PM: ', num2str(noise_cm_prop)])

data_estimated = zeros(size(full_data(:,t_start:t_stop)));
for i_dip = 1:numel(estimated_dipoles)
    if ncomp == 1
        G = leadfield(:,estimated_dipoles(i_dip)*ncomp);
        for t=1:size(data_estimated,2)
            data_estimated(:,t) = data_estimated(:,t) + G*QV_estimated(i_dip*ncomp,t);
        end
    else
        G = leadfield(:,estimated_dipoles(i_dip)*ncomp-2:estimated_dipoles(i_dip)*ncomp);
        for t=1:size(data_estimated,2)
            data_estimated(:,t) = data_estimated(:,t) + G*QV_estimated(i_dip*ncomp-2:i_dip*ncomp,t);
        end
    end
end
...
weights = All_log_Weights_u(best_iter,:)/sum(All_log_Weights_u(best_iter,:));
if best_iter > 1
    [estimated_dipoles_eb, pmap_eb] = estimates(MCsamples{1, best_iter}.all_particles, weights, n_samples, V, NDIP);
else
    estimated_dipoles_eb = [];
    pmap_eb = [];
end
...
% actual results:
result.noise_post = noise_post;
result.noise_std_eff = noise_std_eff;
result.pmap = pmap(1:size(V,1),1:NDIP);
result.pmap_old = pmap_old;
result.pmap_eb = pmap_eb;
result.pmap_singdip = pmap_singdip(:,:,1:n);
result.mod_sel = mod_sel;
result.estimated_dipoles = estimated_dipoles;
result.estimated_dipoles_eb = estimated_dipoles_eb;
result.Q_estimated = Q_estimated;
result.MCsamples = MCsamples;
result.AllWeights = weights;
result.All_log_Weights_u = All_log_Weights_u_eff;% viani
result.log_cost_norm = log_cost_norm;%viani
result.All_log_Weights_u_old_iter = All_log_Weights_u(1:n,:);% viani
...
result.ESS = ESS;
result.best_particle = best_particle_eff;
result.TODAY = date;
result.noise_map_prop = noise_map_prop;
result.noise_cm_prop = noise_cm_prop;
result.cpu_time = cputime - tStart;
result.best_iter = best_iter;
end

function [estimated_dipoles, pmap] = estimates(particle, weights, n_samples, sourcespace, NDIP)

[~, ind_max_weight] = max(weights);
best_particle_eff = particle(ind_max_weight);
mod_sel = zeros(9,1);
pmap = zeros(size(sourcespace,1), 1, NDIP);
for i = 1:n_samples
    mod_sel(particle(i).nu+1) = mod_sel(particle(i).nu+1) + weights(i);
    if particle(i).nu <=NDIP
        for r = 1:particle(i).nu
            pmap(particle(i).dipole(r).c,1, particle(i).nu) = ...
                pmap(particle(i).dipole(r).c,1, particle(i).nu) + weights(i);
        end
    end
end
[~, ind_mod] = max(mod_sel);
%disp(strcat(['Estimated number of dipoles: ', num2str(ind_mod-1)]))
[~, eee,pmap_singdip(:,:,1)] = point_estimation(particle, weights, sourcespace, NDIP);
kkk = 1;
if numel(eee)>0
    for i = 1:numel(eee)
        est_dip(kkk,:) = [sourcespace(eee(i),:) 1 eee(i)];
...
function [particle] = prior_and_like(particle, exponent_likelihood, ...
    leadfield, data, lambda_prior, dipmom_std, nsens, ncomp, fact, noise_std, n_ist)

particle.prior = 1/fact(particle.nu+1) * exp(-lambda_prior) * lambda_prior^particle.nu * 1 / particle.dipmom_std;
...
particle.log_like = 0;
particle.like_det = 1;
if exponent_likelihood > 0
    constant = exponent_likelihood * (dipmom_std/noise_std)^2;
    cov_likelihood_risc = constant *G_r*G_r' + eye(nsens);
    % cov_likelihood_risc_inv = inv(cov_likelihood_risc);
    cov_likelihood_risc_inv = eye(nsens) - constant *G_r * inv(eye(ncomp*particle.nu) + G_r'* constant *G_r)*G_r';
...
function delta = evaluate_delta(noise_std_eff)
delta = zeros(numel(noise_std_eff),1);
delta(1) = abs(noise_std_eff(1) - noise_std_eff(2));
delta(numel(noise_std_eff)) = abs(noise_std_eff(numel(noise_std_eff)-1) - ...
    noise_std_eff(numel(noise_std_eff)));
for i=3:numel(noise_std_eff)
    delta(i-1) = abs(noise_std_eff(i-2) - noise_std_eff(i));
end
end
