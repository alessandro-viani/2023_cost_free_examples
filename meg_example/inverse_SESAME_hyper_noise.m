...
v_noise = zeros(n_samples, N);
v_weight = zeros(n_samples, N);
...
for i = 1:n_samples
    ...
    particle(i).noise_std = 10^(noise_range*rand)*noise_std/35;%gamrnd(2,noise_std * 4);%10^(noise_range*rand)*noise_std/35;
    for r = 1:ndip
...
while exponent_likelihood(n) <= 1
    ...
    for i=1:n_samples
        v_noise(i, n) =  particle(i).noise_std/scaling_factor;
        v_weight(i, n) = weights(i);
    end
    disp(strcat(['Conditional Mean noise std: ', num2str(sum(v_noise(:,n) .* v_weight(:,n)))]));
...
[result.noise_cm_hy, result.noise_map_hy] = noise_estimates(result);
result.v_noise = v_noise(:,1:n);
result.v_weight = v_weight(:,1:n);
...
constant = (particle.dipmom_std/particle.noise_std)^2;
...
function [noise_cm_hy, noise_map_hy] = noise_estimates(posterior)
n_bins = 20;
